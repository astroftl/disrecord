                    ...
                    match x {
                        Ok(zip_path) => {
                            ...
                            if let Err(e) = cmd.create_followup(ctx, followup).await {
                                ...
                                let followup = CreateInteractionResponseFollowup::new().content("Failed to send .zip (file too large?)");
                                if let Err(e) = cmd.create_followup(ctx, followup).await {
                    ...
                    Ok(x) => {
                        match x {
                            Ok(mix_path) => {
                                ...
                                if let Err(e) = cmd.create_followup(ctx, followup).await {
                                    error!("Error sending followup to the interaction: {e:?}");
                                    let followup = CreateInteractionResponseFollowup::new().content("Failed to send mixed .opus (file too large?)");
                                    if let Err(e) = cmd.create_followup(ctx, followup).await {
                                        error!("Error sending followup to explain why the followup failed (ironic): {e:?}");
                                    }
                                }
